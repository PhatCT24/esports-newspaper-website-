...
@router.get("/by-category", response_model=List[PostResponseSchema])
async def get_posts_by_category(category: str = Query(...)):
    return await service.get_posts_by_category(category)

@router.get("/by-subcategory", response_model=List[PostResponseSchema])
async def get_posts_by_subcategory(category: str = Query(...), subcategory: str = Query(...)):
    return await service.get_posts_by_subcategory(category, subcategory)
